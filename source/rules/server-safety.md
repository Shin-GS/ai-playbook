...
changelog: 적용 범위 섹션 추가 — 명령 안내/직접 실행/스크립트 작성 모두 포함 명시
activation: manual
activationPattern: []
dependsOn: []
compatibleWith: []
...
서버/인프라 명령을 다룰 때 "일단 해보자" 대신 "먼저 확인하고 실행"하는 습관을 강제한다.
...
이 규칙은 아래 **모든 경우**에 적용된다:

| 시나리오 | 설명 |
|----------|------|
| AI가 사용자에게 명령어를 안내/제안 | 사용자가 SSH로 복붙 실행하는 경우 (가장 흔한 케이스) |
| AI가 직접 shell 도구로 실행 | 로컬 환경이나 MCP로 서버에 연결된 경우 |
| AI가 스크립트/설정 파일을 작성 | Dockerfile, docker-compose, CI/CD 파이프라인 등 |

**핵심:** 명령을 "누가 실행하는가"는 중요하지 않다. AI가 위험한 명령을 제안하는 시점에 체크가 필요하다.
...
명령을 직접 실행하든, 사용자에게 안내하든 동일한 순서를 따른다:

```
1. [조회] 현재 상태 확인 (안전한 조회 명령 안내/실행)
      ↓
2. [분석] 충돌/의존성/리소스 체크
      ↓
...
4. [승인] 사용자 OK 받기 (🔴 이상)
      ↓
5. [백업] 영향받는 설정/데이터 백업 명령 안내
      ↓
6. [실행] 명령 안내 또는 직접 실행
      ↓
7. [검증] 정상 동작 확인 명령 안내
      ↓
8. [보고] 완료 상태 + 문제 시 롤백 방법 안내
```

### 안내 시 포맷

위험 명령을 제안할 때는 명령어만 던지지 않고 아래를 포함:
- ⚠️ 위험도 표시
- 이 명령이 하는 일 (한 줄)
- 사전 확인 명령 (먼저 이걸 실행해서 확인해보세요)
- 문제 시 롤백 방법
