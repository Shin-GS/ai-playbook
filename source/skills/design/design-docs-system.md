...
## 화면 목적
- 이 화면이 존재하는 이유 (사용자가 여기서 뭘 달성하는지)
- 핵심 성공 지표 (이 화면이 잘 동작하면 무엇이 일어나는지)
...
## 진입 경로
- 어디서 이 화면으로 오는지 (네비게이션 맥락)
- 딥링크/직접 접근 가능 여부

## 데이터 의존성
- 이 화면이 필요로 하는 데이터 (API, 로컬 상태)
- 로딩 순서 (뭐가 먼저 로드되어야 하는지)

---

## Case 1: {케이스명}
- **의도**: 왜 이 케이스가 필요한지 (사용자 시나리오)
- **조건**: 데이터 상태 / 사용자 상태
- **표시**: 화면에 보여야 할 내용
- **인터랙션**: 사용자 행동 → 결과
- **비즈니스 규칙**: business-logic.md의 {섹션명} 참조 (해당 시)
- **에러 처리**: 이 케이스에서 발생 가능한 에러 → 표시 방법
...
## 공통 상태
- **로딩**: 데이터 로드 중 표시 (스켈레톤? 스피너? 어떤 영역?)
- **빈 상태**: 데이터 없을 때 (CTA 포함 여부)
- **오프라인**: 네트워크 없을 때 (해당 시)
- **권한 없음**: 접근 불가 시 (해당 시)
...
| 필드 | 역할 | 왜 필요한지 |
|------|------|------------|
| 화면 목적 | 이 화면이 존재하는 이유 | AI가 설계 판단할 때 "이 화면의 핵심 가치"를 인지 |
| 지원 뷰포트 | 어떤 디바이스에서 동작하는지 | 디자이너가 불필요한 사이즈까지 만들지 않도록 |
| 진입 경로 | 어디서 오는지 | 네비게이션 설계, 뒤로가기 동작 결정 근거 |
| 데이터 의존성 | 뭘 fetch해야 하는지 | FE 개발자가 로딩 순서 파악 |
| 의도 | 각 케이스가 왜 필요한지 | 기획 의도를 코드로 정확히 전달 |
| 비즈니스 규칙 참조 | 어떤 규칙이 적용되는지 | 규칙 변경 시 영향받는 화면 역추적 가능 |
| 공통 상태 | 로딩/빈/에러 | 매번 빠지는 상태를 구조적으로 강제 |
| 이탈 경로 | 어디로 가는지 | 화면 간 흐름 명시 |

### cases.md → html 변환 검증

html 작성 완료 후 아래를 확인:
- [ ] cases.md의 모든 Case가 html에 case-switcher 탭으로 존재하는가
- [ ] 공통 상태(로딩, 빈, 에러)가 html에 구현되었는가
- [ ] 지원 뷰포트에 명시된 사이즈에서 레이아웃이 깨지지 않는가
